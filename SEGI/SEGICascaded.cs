...
public class SEGICascaded : MonoBehaviour
{
	...
	public float vramUsage  //TODO: Update vram usage calculation
	{
		get
		{
			...
			if (sunDepthTexture != null)
				v += sunDepthTexture.width * sunDepthTexture.height * 32;
	...
	void GetBlueNoiseTextures()
	{
		...
		for (int i = 0; i < 64; i++)
		{
			...
			if ((blueNoiseTexture = Bundle.LoadAsset<Texture2D>(fileName)) == null)
	...
	void Init()
	{
		...
		if ((sunDepthShader = Bundle.LoadAsset<Shader>("SEGIRenderSunDepth_C")) == null)
			throw new ApplicationException("Could not find asset \"Hidden/SEGIRenderSunDepth_C\"");
		if ((clearCompute = Bundle.LoadAsset<ComputeShader>("SEGIClear_C")) == null)
			throw new ApplicationException("Could not find asset \"SEGIClear_C\"");
		if ((transferIntsCompute = Bundle.LoadAsset<ComputeShader>("SEGITransferInts_C")) == null)
			throw new ApplicationException("Could not find asset \"SEGITransferInts_C\"");
		if ((mipFilterCompute = Bundle.LoadAsset<ComputeShader>("SEGIMipFilter_C")) == null)
			throw new ApplicationException("Could not find asset \"Hidden/SEGIMipFilter_C\"");

		if ((voxelizationShader = Bundle.LoadAsset<Shader>("SEGIVoxelizeScene_C")) == null)
			throw new ApplicationException("Could not find asset \"Hidden/SEGIVoxelizeScene_C\"");
		if ((voxelTracingShader = Bundle.LoadAsset<Shader>("SEGITraceScene_C")) == null)
			throw new ApplicationException("Could not find asset \"Hidden/SEGITraceScene_C\"");

		if (!material)
		{
			Shader segi;
			if ((segi = Bundle.LoadAsset<Shader>("SEGI_C")) == null)
	...
	void OnPreRender()
	{
		...
		if (renderState == RenderState.Voxelize)
		{
			...
			clearCompute.Dispatch(0, Math.Max(1, activeClipmap.resolution / 16), Math.Max(1, activeClipmap.resolution / 16), 1);
			...
			transferIntsCompute.Dispatch(0, Math.Max(1, activeClipmap.resolution / 16), Math.Max(1, activeClipmap.resolution / 16), 1);
			...
			for (int i = 0 + 1; i < numClipmaps; i++)
			{
				...
				mipFilterCompute.Dispatch(0, Math.Max(1, targetClipmap.resolution / 16), Math.Max(1, targetClipmap.resolution / 16), 1);
			}
		...
		else if (renderState == RenderState.Bounce)
		{
			...
			clearCompute.Dispatch(0, Math.Max(1, (int)voxelResolution / 16), Math.Max(1, (int)voxelResolution / 16), 1);
			...
			if (currentClipmapIndex <= 2)
			{
				...
				transferIntsCompute.Dispatch(1, Math.Max(1, (int)voxelResolution / 16), Math.Max(1, (int)voxelResolution / 16), 1);
			}
