...
public class SEGI : MonoBehaviour
{
	...
	void Init()
	{
		...
		if ((sunDepthShader = Bundle.LoadAsset<Shader>("SEGIRenderSunDepth")) == null)
			throw new ApplicationException("Could not find asset \"Hidden/SEGIRenderSunDepth\"");
		if ((clearCompute = Bundle.LoadAsset<ComputeShader>("SEGIClear")) == null)
			throw new ApplicationException("Could not find asset \"SEGIClear\"");
		if ((transferIntsCompute = Bundle.LoadAsset<ComputeShader>("SEGITransferInts")) == null)
			throw new ApplicationException("Could not find asset \"SEGITransferInts\"");
		if ((mipFilterCompute = Bundle.LoadAsset<ComputeShader>("SEGIMipFilter")) == null)
			throw new ApplicationException("Could not find asset \"Hidden/SEGIMipFilter\"");

		if ((voxelizationShader = Bundle.LoadAsset<Shader>("SEGIVoxelizeScene")) == null)
			throw new ApplicationException("Could not find asset \"Hidden/SEGIVoxelizeScene\"");
		if ((voxelTracingShader = Bundle.LoadAsset<Shader>("SEGITraceScene")) == null)
			throw new ApplicationException("Could not find asset \"Hidden/SEGITraceScene\"");

		if (!material)
		{
			Shader segi;
			if ((segi = Bundle.LoadAsset<Shader>("SEGI")) == null)
	...
	void OnPreRender()
	{
		...
		if (renderState == RenderState.Voxelize)
		{
			...
			//Set paramteters
			var sunColor = sun == null ? Color.black : sun.color;
			var sunIntensity = sun == null ? 0 : sun.intensity;
			...
			transferIntsCompute.Dispatch(0, Math.Max(1, (int)voxelResolution / 16), Math.Max(1, (int)voxelResolution / 16), 1);
			...
			for (int i = 0; i < numMipLevels - 1; i++)
			{
				...
				mipFilterCompute.SetInt(SEGI.destinationRes, destinationRes);
				...
				mipFilterCompute.Dispatch(mipFilterKernel, Math.Max(1, destinationRes / 8), Math.Max(1, destinationRes / 8), 1);
